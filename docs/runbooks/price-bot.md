...
- `OpenPriceFeed.setPriceSnapshot` skips a batch entry whose `updatedAt` is older than the stored snapshot (emitting `PriceSnapshotSkipped`) instead of reverting, so a batch raced by another authorized submitter still lands its fresh entries and the outbox does not retry immutable superseded calldata to exhaustion.
