...
- manual retry plan for verified but undelivered packets when `verified_but_undelivered_count` is non-zero, using txmgr automatic retry and `tx_manager.stale_broadcast_replacement_after_seconds` pending replacement first. Run `go run ./go/cmd/txretry -config <worker.yaml> -action retry-failed|replace -id <tx_outbox_id>` only after automatic retry is exhausted or an operator override is approved. `replace` records an operator replacement request on a broadcast or reprice-held row and resets its signing-failure budget; the tx manager then signs a same-nonce replacement attempt at the greater of the fresh RPC suggestion and a 10% bump over the previous signed fee (deferring instead of exceeding the configured cap) and broadcasts the persisted raw. `retry-failed` requeues estimate-gas failures in place; only receipt-failed rows are cloned to keep mined failure evidence, and lzReceive receipt retries are skipped when the executor workflow has already advanced. Rows in `status = held` with reason `manual` or `nonce_reconcile_required` stay parked until nonce reconciliation or an operator cancel.
