...
- Tx fees are selected at send time by `txmgr`, which estimates gas, reads current RPC fee suggestions, applies configured caps, and signs. Every signed transaction is persisted as an immutable `tx_attempts` row before any node sees it: broadcasts, bounded same-raw replays, and same-nonce replacements after `tx_manager.stale_broadcast_replacement_after_seconds` all send previously persisted raw bytes, so an ambiguous send result or a crash mid-broadcast can never lose receipt tracking for a possibly accepted transaction. A node's deterministic rejection, a `nonce too low` response, or an exhausted retry budget parks the signer lane as `held` for operator review instead of releasing the nonce; one signer processes one in-flight nonce at a time. A mined receipt's terminal workflow state is applied only once the receipt is buried under the chain's configured confirmation depth, mirroring the indexer, so a short reorg cannot leave a terminal database state for a transaction the chain rolled back.
